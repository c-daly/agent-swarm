...
def load_data():
    ...
    if mnist_data is None:
        ...
        print(
            f"MNIST loaded: {X_train.shape[0]} training samples, {X_test.shape[0]} test samples"
        )
...
def create_network():
    ...
    return jsonify(
        {"message": "Network created successfully", "layer_sizes": layer_sizes}
    )
...
def start_training():
    ...
    return jsonify(
        {"message": "Training started", "epochs": epochs, "batch_size": batch_size}
    )
