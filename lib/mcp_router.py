...
try:
    ...
    def load_telemetry_v2(path):
        return {}  # type: ignore

    def save_telemetry_v2(data, path):
        pass  # type: ignore

    def ensure_day(data, key):
        return data.setdefault(key, {})  # type: ignore

    def update_timing_stats(timing, backend, duration):
        pass  # type: ignore
...
class TelemetryCollector:
    ...
    def __init__(
        ...
        self._telemetry_file = (
            telemetry_file or Path.home() / ".claude/plugins/agent-swarm/.state/telemetry.json"
        )
    ...
    def track_request(self, destination: str, tool_name: str, args: dict) -> str:
        ...
        correlation_id = (
            f"req-{int(time.time() * 1000)}-{hash((destination, tool_name)) & 0xFFFF:04x}"
        )

        with self._lock:
            self._pending_requests[correlation_id] = (tool_name, destination, args, time.time())
...
class MCPRouter:
    ...
    def register_server(
        self, name: str, command: list[str], args: Optional[dict] = None, tool_prefix: str = ""
    ) -> dict:
        ...
        with self._lock:
            config = ServerConfig(
                name=name, command=command, args=args or {}, tool_prefix=tool_prefix
            )
    ...
    def unregister_server(self, name: str) -> dict:
        ...
        with self._lock:
            ...
            if proc:
                proc.terminate()
            return {"status": "unregistered" if existed else "not_found", "name": name}
            ...
            try:
                ...
                if getattr(e, "errno", None) == 98:  # EADDRINUSE
                    ...
                    if main_port:
                        ...
                        _router_log(
                            "federation", f"Lost race, joined as secondary on port {main_port}"
                        )
                        return main_port
                else:
                    raise
        ...
        while self._socket_running:
            try:
                ...
                try:
                    ...
                    _router_log(
                        "socket",
                        f"ACCEPT conn_id={conn_id} from={addr} active={self._active_socket_connections}",
                    )
                    ...
                    def handle_and_track(c, cid):
                        try:
                            ...
                            _router_log(
                                "socket",
                                f"CLOSE conn_id={cid} active={self._active_socket_connections}",
                            )

                    Thread(target=handle_and_track, args=(client, conn_id), daemon=True).start()
                except socket.timeout:
                    continue
            except Exception as e:
    ...
    def _handle_socket_client(self, client: socket.socket, conn_id: int = 0) -> None:
        ...
        try:
            ...
            while b"\n" not in data:
                ...
                if not chunk:
                    _router_log(
                        "socket", f"{log_prefix} Client disconnected before sending data", "WARN"
                    )
                    return
                data += chunk
            ...
            _router_log(
                "socket",
                f"{log_prefix} Received {len(data)} bytes in {int((read_time - start_time) * 1000)}ms",
            )
            ...
            if method == "tools/list":
                ...
                _router_log(
                    "socket",
                    f"{log_prefix} COMPLETE tools/list duration={duration_ms}ms tools_count={len(tools)}",
                )
                return
                ...
                if tool_name == "router__ping":
                    ...
                    if self.telemetry:
                        ...
                        summary["federation"] = {
                            ...
                            "main_port": self._main_router_port,
                        }
                        result = {
                    ...
                    result = {
                        "content": [
                            {
                                "type": "text",
                                "text": json.dumps({"tools": [t["name"] for t in tools]}, indent=2),
                            }
                        ]
                    }
                    ...
                    if not destination:
                        _router_log(
                            "socket", f"{log_prefix} Unknown backend prefix: {prefix}", "ERROR"
                        )
                        result = {
                            "content": [
                                {"type": "text", "text": f"Error: Unknown backend prefix: {prefix}"}
                            ],
                            "isError": True,
                        }
                    else:
                        ...
                        if isinstance(backend_result, dict) and "error" in backend_result:
                            _router_log(
                                "socket",
                                f"{log_prefix} Backend error: {str(backend_result.get('error', ''))[:100]}",
                                "ERROR",
                            )
                            result = {
                                "content": [
                                    {"type": "text", "text": f"Error: {backend_result['error']}"}
                                ],
                                "isError": True,
                            }
                        else:
                    ...
                    _router_log(
                        "socket", f"{log_prefix} Tool name not prefixed: {tool_name}", "ERROR"
                    )
                    result = {
                        "content": [
                            {"type": "text", "text": "Tool name must be prefixed: prefix__tool"}
                        ],
                        "isError": True,
                    }
            else:
            ...
            _router_log(
                "socket",
                f"{log_prefix} COMPLETE tool={tool_name} duration={duration_ms}ms response_size={len(response_bytes)}",
            )
            if self.telemetry:
            ...
            duration_ms = int((time.time() - start_time) * 1000)
            _router_log(
                "socket", f"{log_prefix} TIMEOUT tool={tool_name} duration={duration_ms}ms", "ERROR"
            )
            if self.telemetry:
            ...
            _router_log(
                "socket",
                f"{log_prefix} ERROR tool={tool_name} duration={duration_ms}ms error={str(e)[:100]}",
                "ERROR",
            )
            if self.telemetry:
    ...
    def _track_socket_event(
        self, status: str, tool_name: str, duration_ms: int, error_msg: str = ""
    ) -> None:
            ...
            # Event system pseudo-tools (handled by hooks, not router)
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        "tool": {
                            "type": "string",
                            "description": "Tool name (e.g., serena__find_symbol)",
                        },
            ...
            },
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        "correlation_id": {
                            "type": "string",
                            "description": "Correlation ID from router__request",
                        }
                    },
    ...
    def route(
                ...
                if isinstance(result, dict) and "content" in result:
                    ...
                    if content and isinstance(content[0], dict):
                        ...
                        try:
                            ...
                            response = RouterResponse(
                                summary=text[:200], full=result, correlation_id=correlation_id
                            )
                    else:
                        response = RouterResponse(
                            summary=str(result)[:200], full=result, correlation_id=correlation_id
                        )
                else:
                    response = RouterResponse(
                        summary=str(result)[:200], full=result, correlation_id=correlation_id
        ...
        if not server:
            ...
            response = RouterResponse(
                summary=summary, full=full_response, correlation_id=correlation_id
            )
    ...
    def _generate_correlation_id(self, destination: str, tool_name: str, args: dict) -> str:
        """Generate correlation ID from request parameters.

        Uses hash of args for deterministic IDs (same request = same ID).
        """
        content = json.dumps(
            {"destination": destination, "tool": tool_name, "args": args}, sort_keys=True
        )
    ...
    def _forward_to_server(
        self, server: ServerConfig, tool_name: str, args: dict, _is_restore: bool = False
    ) -> dict:
        ...
        with backend_lock:
            try:
                ...
                if not ready:
                    ...
                    return {
                        "error": {"code": -32603, "message": f"Timeout waiting for {server.name}"}
                    }
...
JSON:"""
    ...
    def _fallback_extract(self, tool_name: str, content: str) -> str:
        ...
        # Generic fallback
        return json.dumps(
            {"char_count": len(content), "preview": content[:500], "truncated": len(content) > 500}
        )
...
def analyze_telemetry_trends(telemetry: TelemetryCollector) -> dict:
    ...
    high_call_tools = [
        (tool, count) for tool, count in by_tool.items() if isinstance(count, int) and count > 50
    ]
    if high_call_tools:
        sorted_tools = sorted(high_call_tools, key=lambda x: x[1], reverse=True)[:3]
        analysis["top_tool_callers"] = [{"tool": t, "calls": v} for t, v in sorted_tools]
        analysis["recommendations"].append(
            ...
            },
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        "workflow_id": {
                            "type": "string",
                            "description": "Workflow ID to clear (optional, omit for all)",
                        }
                    },
                },
            },
    ...
    for line in sys.stdin:
        ...
        try:
                ...
                # Parse prefix__toolname for backend routing
                    ...
                    if not destination:
                        ...
                        if "error" in full_result:
                            result = {
                                "content": [
                                    {"type": "text", "text": f"Error: {full_result['error']}"}
                                ],
                                "isError": True,
                            }
                        else:
                            result = {
                                "content": [{"type": "text", "text": json.dumps(full_result)}]
                            }
                    else:
            ...
            if request_id is not None:
                response_msg = {"jsonrpc": "2.0", "id": request_id, "result": result}
