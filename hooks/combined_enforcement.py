...
# Bash patterns allowed for orchestrator (queue/workflow management only)
ORCHESTRATOR_ALLOWED_BASH_PATTERNS = [
    r"^python3?\s+.*workflow\.py\s+(status|advance|iterate|orchestrate|reset|stop)",
    r"^python3?\s+.*iterate_state\.py",
    r"^cat\s+.*\.state/.*\.json",
    r"^gh\s+pr\s+(list|view|create|comment)",
    r"^gh\s+api\s+",
...
def check_orchestrator_restrictions(tool_name: str, tool_input: dict, state: dict, input_data: dict = None) -> dict | None:
    """Block implementation tools when in orchestrate mode.

    The orchestrator should spawn subagents for implementation work,
    not do it directly. This enforces that pattern.

    Subagents have their own isolated state files (via StateManager),
    so they never see mode='orchestrate'. Only the orchestrator itself
    has mode='orchestrate' in its state.
    """
    tool_input, state = _validate_inputs(tool_input=tool_input, state=state)
    ...
    # Handle Bash specially - only allow whitelisted patterns
    if tool_name == "Bash":
        command = tool_input.get("command", "").strip()
        for pattern in ORCHESTRATOR_ALLOWED_BASH_PATTERNS:
            if re.match(pattern, command):
                return None  # Whitelisted command
        return block(
            f"[ORCHESTRATOR] Bash command not whitelisted.\n\n"
            f"Allowed patterns for orchestrator:\n"
            f"  - workflow.py commands (status, advance, etc.)\n"
            f"  - iterate_state.py (queue management)\n"
            f"  - cat .state/*.json (read state)\n"
            f"  - gh pr/api (PR management)\n"
            f"  - git status/log/diff/branch/show (read-only git)\n\n"
            f"For implementation commands, spawn a subagent."
        )
