    ...
    if "--json" not in args and "-q" not in args:
        ...
        if any(cmd_str.startswith(jc) for jc in json_commands):
            if "list" in args:
                ...
                cmd.extend(
                    ["--json", "number,title,state,body,author,createdAt,comments"]
                )
...
def summarize_pr_list(data: list) -> str:
    ...
    for pr in data[:10]:
        lines.append(
            f"  #{pr.get('number', '?')} [{pr.get('state', '?')}] {pr.get('title', 'No title')[:60]}"
        )
...
def summarize_issue_list(data: list) -> str:
    ...
    for issue in data[:10]:
        lines.append(
            f"  #{issue.get('number', '?')} [{issue.get('state', '?')}] {issue.get('title', 'No title')[:60]}"
        )
