...
def extract_imports(content: str, ext: str) -> list[str]:
    ...
    for line in content.split('\n'):
        match = re.match(pattern, line.strip())
        if match:
            imp = match.group(1) or match.group(2) if match.lastindex >= 2 else match.group(1)
            if imp:
                imports.append(imp)
...
def format_output(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        if r.get("functions"):
            ...
            lines.append(f"  Functions ({len(r['functions'])}): {', '.join(f['name'] for f in funcs)}")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: file_analyzer.py '{\"files\": [...], \"mode\": \"summary|functions|imports\"}'")
        sys.exit(1)
